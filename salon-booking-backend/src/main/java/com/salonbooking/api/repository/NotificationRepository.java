...
public interface NotificationRepository extends JpaRepository<Notification, UUID> {
    ...
    // Fetch for specific Customer (includes broadcast messages where receiverId is null)
    @org.springframework.data.jpa.repository.Query("SELECT n FROM Notification n WHERE n.receiverType = :receiverType AND (n.receiverId = :receiverId OR n.receiverId IS NULL) ORDER BY n.createdAt DESC")
